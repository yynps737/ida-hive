...
json cmd_survey_binary(const json &params)
{
...
json cmd_analyze_component(const json &params)
{
    ...
    for (ea_t fea : component_eas)
    {
        ...
        for (const call_edge_t &e : collect_callees(f->start_ea, f->end_ea))
            callees_list.push_back({
                ...
                {"internal", component_eas.count(e.callee_ea) > 0},
            });
...
const command_entry_t kCommands[] = {
    ...
    { "survey_binary",     cmd_survey_binary },
    { "trace_data_flow",   cmd_trace_data_flow },
    { "analyze_component", cmd_analyze_component },
    { "diff_before_after", cmd_diff_before_after },
};
